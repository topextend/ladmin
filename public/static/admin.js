...
$(function () {
    ...
    /*! 注册 data-export 表单导出行为 */
    $body.on('click', '[data-export]', function () {
        var url = $('form.form-search').attr('export').replace(/&?page=\d+/g, ''), split = url.indexOf('?') === -1 ? '?' : '&';
        if (($('form.form-search').method || 'get').toLowerCase() === 'get') {
            return window.open($.menu.parseUri(url + split + $('form.form-search').serialize()));
        }
        $.form.load(url, this, 'post');
    });
